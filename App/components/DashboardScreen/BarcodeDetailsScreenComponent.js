...
const BarcodeDetailsScreenComponent = props => {
  const {data, bagID, bagTypes, bag_Name, navigation} = props;
  const systemFonts = [
    'Inter-Black',
    'Inter-Bold',
    'Inter-ExtraBold',
  ...
  const [photoErrorBox, setPhotoErrorBox] = useState(false);
  const [errorMessage, setErrorMessage] = useState('');
  const [screenNames, setScreenNames] = useState('');
  const contentWidth = useWindowDimensions().width;

  useEffect(() => {
    console.log('setSelectedIndex==> ' + JSON.stringify(props.bagTypes));
    setBagType(bagTypes - 1);
    setSelectedIndex(bagTypes - 1);
    //setSelectedIndex(bagTypes);
    setBagType(bagTypes);
    PrefManager.getValue('@bag_ID').then(id => {
      setBagID(id);
    });

    //setSelectedIndex(data?.data?.bag_type - 1);
    setSelected(data?.data?.bag_weight);
  ...
  const callUsingBagID = async (orderID, id) => {
    ...
    //         setScreenNames('');
    // Platform.OS === 'ios' ? Alert.alert("", res?.data && res?.data?.error) :  setErrorBoxOpen(true);
    //         setErrorBoxOpen(true);
    //         setErrorMessage(res?.data && res?.data?.error);
    //       }
    //     })
  ...
  return (
    <ScrollView>
      <LinearGradient
        ...
        {/* <FlatList
          data={BagList}
          keyExtractor={item => `menu-list-${item}`}
            ...
            <View
              ...
              <Button
                ...
                onPress={() => {
                  selectedIndex !== ''
                    ? PrefManager.getValue('@bag_ID').then(id => {
                        // callUsingBagID(data?.data?.order_id, bagID);
                        callUsingSaveNext(data?.data?.order_id, bagID);
                      })
