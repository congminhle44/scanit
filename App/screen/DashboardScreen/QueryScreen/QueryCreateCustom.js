...
const QueryCreateCustom = props => {
  const [navigationa, setNavigation] = useState('LoginScreen');
  const [userName, setUserName] = useState('');
  const {navigation, data, bagID, bag_type, bag_Name} = props.route.params;
  const [bagType, setBagType] = useState(bag_type - 1);
  ...
  const [selectedIndex, setSelectedIndex] = useState(bag_type - 1);
  ...
  const callUsingBagID = async (orderID, id) => {
    var bagName =
      props.schooldetail &&
      props.schooldetail.filter(item => item.typed === 'transit').map(x => x);
    let defaultLocationa =
      bag_Name === 'identify' ? props.schooldetail[0]?.id : bagName[0]?.id;

    var data = {
      // bag_id: JSON.parse(id),
      bag_id: id,
      bag_notes: '',
      bag_type: selectedIndex + 1,
      bag_weight: selected,
      baglocation: defaultLocationa,
      forceugly: 1,
      new_order_id: null,
      newstatus: null,
      order_id: orderID,
    ...
    if (isConnected == true) {
      ...
      axios(config).then(res => {
        ...
        if (
        ...
        } else if (res && res?.data && res?.data?.ret === 2) {
          ...
          setErrorBoxOpen(true);
          setErrorMessage(res?.data && res?.data?.error);
        } else if (res && res?.data && res?.data?.ret === 4) {
          setScreenNames('');
          setErrorBoxOpen(true);
          setErrorMessage(res?.data && res?.data?.error);
        }
  ...
  const ImageSelection = async id => {
    ImagePicker.openCamera({
      ...
      data.append('files', {
        uri: image.path,
        name: id + '_' + image.path.split('/').pop(),
        type: image.mime,
      });
          ...
          <View style={{marginTop: 10}}>
            ...
            <Button
              ...
              onPress={() => {
                selectedIndex !== ''
                  ? PrefManager.getValue('@bag_ID').then(id => {
                      callUsingSaveNext(data?.order_id, id);
                    })
                  : Alert.alert('', 'Please select radio button.');
              }}
              textStyle={{fontSize: TextFontSize.TEXT_SIZE_MEDIUM}}
...
export default connect(mapStateToProps, mapDispatchToProps)(QueryCreateCustom);
