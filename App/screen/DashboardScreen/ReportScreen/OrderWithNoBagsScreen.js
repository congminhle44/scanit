...
const OrderWithNoBagsScreen = props => {
  ...
  useEffect(() => {
    props.getReport(`?school=${schoolID}&house=${houseID}&type=3`);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    <View style={{backgroundColor: Colors.lightPinkColor}}>
      ...
      <ReportHeader title="Order with no bags" navigation={navigation} />
      ...
      {/* <FlatList
        ...
        extraData={props.reportData}
        contentContainerStyle={{padding: 10, backgroundColor: Colors.white}}
        ListFooterComponent={() => {
          return (
            <View
              style={{
                height: ScaleSizeUtils.LISTING_VERY_SMALL_SIZE,
                width: '100%',
              }}></View>
          );
        }}
